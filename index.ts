...
class User {
  ...
  public changeEmail(newEmail: string): boolean {
    if (!newEmail.includes("@")) {
      console.log("Noto‘g‘ri email formati! Emailda @ belgisi bo‘lishi shart.");
      return false;
    }
...
user.changeEmail("test@gmail");
user.changeEmail("@test.com");
console.log(user.getEmail());
...
class SecureUserAccount {
  ...
  private setEmail(value: string) {
    if (!value.includes("@")) {
      throw new Error(
        "Noto‘g‘ri email formati! Emailda @ belgisi bo‘lishi shart."
      );
    }
    this._email = value;
  }
...
const account = new SecureUserAccount(
  "john_doe",
  "john@example.com",
  "secret123"
);
